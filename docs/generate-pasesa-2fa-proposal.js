// Genera el documento Word con la propuesta de simplificación del endpoint 2FA para PASESA.
// Uso: node generate-pasesa-2fa-proposal.js
// Salida: ./Propuesta-Simplificacion-2FA-PASESA.docx
...
const problemsTable = new Table({
  ...
  rows: [
    ...
    new TableRow({ children: [
      ...
      cell(plain('Cada corredor que use el flujo 2FA debe configurar manualmente en el Webhook Builder la regla "status=CODIGO_GENERADO ⇒ ejecutar SEND_2FA_CODE_EMAIL". Olvidarla deja al cliente sin email y sin mensaje de error visible.'), { width: 4400 }),
      cell(p([t('Media', { bold: true, color: AMBER })]), { width: 2760 }),
    ]}),
    new TableRow({ children: [
...
children.push(numbered([bold('Fase 4 — Cutover PASESA: '), t('en una ventana acordada, en el template de campaña de PASESA: (a) reemplazar las 3 acciones por las 2 nuevas, (b) actualizar el SystemPrompt para que el agente conozca los nuevos slugs, (c) eliminar la ChainRule. El cambio es instantáneo — el siguiente turno usa el flujo nuevo.')]));
...
children.push(new Paragraph({
  alignment: AlignmentType.CENTER,
  children: [t('Documento técnico — AgentFlow / TalkIA — Mayo 2026', { italics: true, color: '64748B', size: 18 })],
}));

// ---- documento ----
const doc = new Document({
  creator: 'AgentFlow',
  title: 'Propuesta de Simplificación — Endpoints 2FA PASESA',
  description: 'Consolidación de 3 endpoints en 2 para el flujo 2FA del broker PASESA',
  styles: {
...
const outPath = path.join(__dirname, 'Propuesta-Simplificacion-2FA-PASESA.docx');
