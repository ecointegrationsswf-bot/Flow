...
export function AdminLoginPage() {
  ...
  const onChangePassword = async (data: ChangePwForm) => {
    setError(null); setLoading(true)
    try {
      const { data: res } = await adminClient.post('/admin/change-password', { tempToken, newPassword: data.newPassword })
      if (res.requires2FA) {
        setTempToken(res.tempToken); setMaskedEmail(res.email); setStep('verify2fa')
      }
    } catch (err: unknown) {
  ...
  const onVerify2FA = async () => {
    ...
    setError(null); setLoading(true)
    try {
      const { data: res } = await adminClient.post('/admin/verify-2fa', { tempToken, code: otpCode })
      finishLogin(res)
    } catch (err: unknown) {
      const a = err as { response?: { data?: { error?: string } } }
  ...
  const onResend = async () => {
    try {
      await adminClient.post('/admin/resend-2fa', { tempToken, code: '' })
      setError(null)
    } catch { /* ignore */ }
  }

  const finishLogin = (res: { token: string; user: { id: string; fullName: string; email: string; role: string } }) => {
    localStorage.setItem('sa_token', res.token)
    localStorage.setItem('sa_user', JSON.stringify(res.user))
    useSuperAdminStore.setState({ token: res.token, user: res.user, isAuthenticated: true })
    navigate('/admin/tenants', { replace: true })
  }

  const inputClass = "block w-full rounded-md border border-gray-600 bg-gray-700 px-3 py-2 text-sm text-white placeholder-gray-400 focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500"

  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-900">
      <div className="w-full max-w-sm rounded-lg bg-gray-800 p-8 shadow-md">
        <div className="mb-6 flex flex-col items-center">
          <img src="/jamcst.png" alt="JamCST" className="h-16 w-16 rounded" />
          <h1 className="mt-3 text-xl font-bold text-white">Admin Panel</h1>
          <p className="text-sm text-gray-400">Acceso de administrador</p>
        </div>

        {error && <div className="mb-4 rounded-md bg-red-900/40 p-3 text-center text-sm text-red-300">{error}</div>}
        ...
        {step === 'login' && (
          <form onSubmit={loginForm.handleSubmit(onLogin)} className="space-y-4">
            <div>
              ...
              <input type="email" {...loginForm.register('email')} className={inputClass} placeholder="admin@talkia.com" />
            ...
            <div>
              ...
              <div className="relative mt-1">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute inset-y-0 right-0 flex items-center pr-3 text-gray-400 hover:text-gray-300">
                  {showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
                </button>
              </div>
            </div>
            <button type="submit" disabled={loading || isLocked} className="flex w-full items-center justify-center gap-2 rounded-md bg-amber-500 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-amber-400 disabled:opacity-50">
        ...
        {step === 'changePassword' && (
          <form onSubmit={changePwForm.handleSubmit(onChangePassword)} className="space-y-4">
            <div className="flex items-center gap-2 rounded-md bg-amber-900/30 p-3 text-sm text-amber-300">
              <KeyRound className="h-5 w-5 shrink-0" />
              Debes cambiar tu contrasena.
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300">Nueva contrasena</label>
              <div className="relative mt-1">
              ...
              {pwValue && (() => { const s = getStrength(pwValue); return (
                <div className="mt-1.5">
                  <div className="h-1.5 w-full rounded-full bg-gray-600"><div className={`h-1.5 rounded-full transition-all ${s.color} ${s.width}`} /></div>
                  <p className={`mt-0.5 text-xs ${s.text}`}>{s.label}</p>
                </div>
              )})()}
              {changePwForm.formState.errors.newPassword && <p className="mt-1 text-xs text-red-400">{changePwForm.formState.errors.newPassword.message}</p>}
            </div>
            <div>
              ...
              {changePwForm.formState.errors.confirmPassword && <p className="mt-1 text-xs text-red-400">{changePwForm.formState.errors.confirmPassword.message}</p>}
            </div>
            <button type="submit" disabled={loading} className="flex w-full items-center justify-center gap-2 rounded-md bg-amber-500 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-amber-400 disabled:opacity-50">
              {loading && <Loader2 className="h-4 w-4 animate-spin" />}
              Cambiar contrasena
            </button>
          </form>
        ...
        {step === 'verify2fa' && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 rounded-md bg-blue-900/30 p-3 text-sm text-blue-300">
              <ShieldCheck className="h-5 w-5 shrink-0" />
              Codigo enviado a {maskedEmail}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300">Codigo de 6 digitos</label>
              <input
            ...
            <button onClick={onVerify2FA} disabled={loading || otpCode.length !== 6} className="flex w-full items-center justify-center gap-2 rounded-md bg-amber-500 px-4 py-2 text-sm font-medium text-gray-900 hover:bg-amber-400 disabled:opacity-50">
              {loading && <Loader2 className="h-4 w-4 animate-spin" />}
              Verificar
            </button>
            <button onClick={onResend} className="w-full text-center text-xs text-gray-500 hover:text-amber-400">
              Reenviar codigo
            </button>
          </div>
        )}
        ...
        <div className="mt-4 text-center">
          <button type="button" onClick={() => navigate('/login')} className="text-xs text-gray-500 hover:text-gray-300">
